...
var AIM9 = {
	new : func (p, mty) {
		...
		m.thrust_duration_1 = getprop("sim/model/f15/systems/armament/"~m.type~"/stage-1-duration-sec");
		m.thrust_duration_2 = getprop("sim/model/f15/systems/armament/"~m.type~"/stage-2-duration-sec");
...
print("Model ",missile_model);
	...
	update: func {
		...
		#print(sprintf("mach %.1f", speed_m)~sprintf(" - time %.1f", me.life_time)~" - thrust "~f_lbs);
            ...
            if(me.life_time > 3)
            {
                ...
                if ( me.free == 0 ) 
                {
                    ...
                    if ( g > me.max_g_current )  
                    {
                        ...
                        me.free = 1;
                        printf("%s: Target unreachable would exceed G %d (max=%d)",me.type,g,me.max_g_current);
...
# do not set launch light when missile tracking.
		...
		if (me.status == 1) {		
			...
			} elsif (t_elev_deg < 0 and me.life_time < me.thrust_duration_1 +me.thrust_duration_2 and t_dist_m * M2NM > cruise_minimum) {
			...
			if (h_gain != 0 and me.dist_last != nil) {
					...
					dev_h = acc_to_deg(me.old_speed_horz_fps, dt_, acc_sideways_ftps2);
					#dev_h = 0;
					...
					if (cruise_or_loft == 0 and me.last_cruise_or_loft == 0) {
						...
						dev_e = acc_to_deg(me.old_speed_fps, dt_, acc_upwards_ftps2);
